...
-   installierten von [`git`](https://git-scm.com/downloads)
...
Soll eine überwachte Datei gesichert werden erfolgt das mit dem Befehl `git commit`. Der Befehl, `git push` wird dann verwendet wenn die bestätigten Verzeichnis-Snapshot in ein entferntes Repository (z.B. Github, GitLab) übertragenm werden sollen.

```{mermaid}
%%| fig-cap: Ablauf Stage-Commit-Push  eine Datei

graph LR
    Z(Erzeuge eine <br> neue Datei)
    
    A[Lokales Repository] -- git add 'Dateiname' --> B[Datei im <br>Staging Area]
    B -- git commit --> C[Dateiversion im <br> Lokalen Repository ]
    C -- git push --> D[Dateiversion im <br> Remote Repository ]
    
    AA[Initialisiers <br> Repository] -.-> BB(git überwacht die Datei)
...
Basierend auf den Informationen aus dem vorausgegangenen Kapitel lernst Du in dieser Lektion: 
...
### Praktische Anwendung von git und GitHub
